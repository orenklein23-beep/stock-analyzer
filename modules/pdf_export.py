...
class PDF(FPDF):
    ...
    def header(self):
        ...
        self.set_xy(10, 6)
        self.cell(0, 10, f"Stock Analyzer — {self.ticker} Report", ln=False)
    ...
    def metric_row(self, items):
        ...
        for label, value in items:
            ...
            self.cell(col_w - 4, 5, str(label), ln=True)
            self.set_font("Helvetica", "B", 10)
            self.set_text_color(20, 20, 40)
            self.set_xy(x_start + 2, y_start + 7)
            self.cell(col_w - 4, 6, str(value), ln=True)
            x_start += col_w
    ...
    def add_image_from_fig(self, fig, w=190, h=80):
        try:
            ...
            self.set_text_color(150,150,150)
            self.cell(0, 8, f"[Chart unavailable: {e}]", ln=True)
            self.set_text_color(0,0,0)


def show(ticker):
    st.header(f"📄 {ticker} — PDF Export")
    ...
    if st.button("🖨️ Generate PDF Report", type="primary"):
        with st.spinner("Building your report... this takes about 30 seconds"):
            try:
                ...
                sentiment_label = "Bullish" if avg_s > 0.1 else "Bearish" if avg_s < -0.1 else "Neutral"
                ...
                pdf.cell(0, 12, f"{ticker} — Analyst Report", ln=True)
                pdf.set_font("Helvetica","",11)
                pdf.set_text_color(80,80,80)
                pdf.cell(0, 7, f"Prepared by: {analyst_name}", ln=True)
                pdf.cell(0, 7, f"Generated: {datetime.now().strftime('%B %d, %Y at %H:%M')}", ln=True)
                pdf.ln(4)
                ...
                pdf.cell(80, 8, f"${round(price,2)}", ln=False)
                pdf.set_font("Helvetica","",11)
                chg_str = f"{direction} ${abs(chg)} ({chgp}%) today"
                pdf.cell(100, 8, chg_str, ln=True)
                ...
                if include_fundamentals:
                    ...
                    if include_charts and not fin.empty:
                        try:
                            ...
                            fig_rev = go.Figure()
                            if "Total Revenue" in f2.columns:
                                fig_rev.add_trace(go.Bar(x=f2.index, y=f2["Total Revenue"],
                                    name="Revenue", marker_color="#4A90D9"))
                            if "Gross Profit" in f2.columns:
                                fig_rev.add_trace(go.Bar(x=f2.index, y=f2["Gross Profit"],
                                    name="Gross Profit", marker_color="#9B59B6"))
                            if "Net Income" in f2.columns:
                                fig_rev.add_trace(go.Bar(x=f2.index, y=f2["Net Income"],
                                    name="Net Income", marker_color="#27AE60"))
                ...
                if include_earnings:
                    ...
                    if earn_dates is not None and not earn_dates.empty:
                        try:
                            ...
                            avg_surp = round(ed["Surprise %"].mean(),1)

                            pdf.metric_row([
                                ("Quarters Tracked", len(ed)),
                                ("Beats",            beats),
                                ("Misses",           misses),
                                ("Avg EPS Surprise", f"{avg_surp}%"),
                            ])
                            pdf.ln(4)
                            ...
                            if include_charts:
                                ...
                                fig_eps.update_layout(height=500, paper_bgcolor="white",
                                    plot_bgcolor="#f8f8f8", title="EPS Actual vs Estimate",
                                    legend=dict(orientation="h"))
                ...
                if include_sentiment and articles:
                    ...
                    pdf.metric_row([
                        ...
                        ("Positive Articles", pos_c),
                        ("Negative Articles", neg_c),
                    ])
                    pdf.ln(4)

                    # Behavioral flag text
                    ...
                    if avg_s > 0.3:
                        flag_text = "Extreme Optimism — Sentiment is very bullish. Good news may already be priced in."
                    elif avg_s > 0.1:
                        flag_text = "Moderately Bullish — Positive sentiment with room to run."
                    elif avg_s < -0.3:
                        flag_text = "Extreme Pessimism — Contrarian opportunity may exist if fundamentals are intact."
                    elif avg_s < -0.1:
                        flag_text = "Moderately Bearish — Negative sentiment building. Monitor for a reversal catalyst."
                    else:
                        flag_text = "Neutral Sentiment — No strong directional bias in recent news."
                    pdf.multi_cell(0, 6, flag_text)
                    pdf.ln(3)
                    pdf.set_text_color(0,0,0)
                    ...
                    if include_charts:
                        try:
                            ...
                            fig_sent.update_layout(height=400, paper_bgcolor="white",
                                title="Sentiment Breakdown")
                            pdf.add_image_from_fig(fig_sent, h=70)
                        except: pass
                    ...
                    for a in articles[:12]:
                        emoji = "+" if a["label"]=="Positive" else "-" if a["label"]=="Negative" else "o"
                        pdf.set_font("Helvetica","",8)
                        label_color = (39,174,96) if a["label"]=="Positive" else \
                                      (231,76,60)  if a["label"]=="Negative" else (100,100,100)
                        pdf.set_text_color(*label_color)
                        pdf.cell(8,6,f"[{emoji}]",ln=False)
                        pdf.set_text_color(40,40,40)
                        title_short = a["title"][:90]+"..." if len(a["title"])>90 else a["title"]
                        pdf.multi_cell(0,6, title_short)
                ...
                with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as tmp:
                    pdf.output(tmp.name)
                    with open(tmp.name, "rb") as f_out:
                ...
                st.download_button(
                    ...
                    data=pdf_bytes,
                    file_name=f"{ticker}_analyst_report_{datetime.now().strftime('%Y%m%d')}.pdf",
                ...
                st.info("Try unchecking 'Include Charts' if the error persists — charts require the kaleido package.")
